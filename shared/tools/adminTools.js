/**
 * Admin Chat Tools - Shared Module
...
import { getProductAnalytics } from '../../src/09-product.js';
import { getDistinctValues } from '../../src/10-get-distinct-values.js';
import { getProductSearchResults } from '../../src/03-search.js';
import { getTagSuggestions } from '../../src/04-get-tag-suggestions.js';
import { prompt } from '../../src/06-prompt.js';
import { relate } from '../../src/07-relate.js';
import { predictInvoice } from '../../src/08-predict-invoice.js';
...
async function getQueryExamples(category = 'basic') {
  // ... (keeping the full examples object from original file)
  // This is a long function with many examples, so I'll keep it the same
  // but update the function name and structure to work in shared module
  try {
    const examples = {
      basic: {
 ...
 */
export const adminToolFunctions = {
  get_user_analytics: getUserAnalytics,
  get_product_performance: getProductPerformance,
  get_inventory_insights: getInventoryInsights,
  search_products_admin: searchProductsAdmin,
  get_customer_support_insights: getCustomerSupportInsights,
  get_business_reports: getBusinessReports,
  generate_product_tags: generateProductTags,
  ...
  execute_aito_query: executeAitoQuery,
  get_query_examples: getQueryExamples,
  validate_aito_query: validateAitoQuery,
};
...
export async function executeAdminTool(toolName, parameters) {
  ...
  const toolFunction = adminToolFunctions[toolName];
  ...
  if (toolName === 'get_user_analytics') {
    return await toolFunction(parameters.timeframe);
  } else if (toolName === 'get_product_performance') {
    return await toolFunction(parameters.productId, parameters.category);
  } else if (toolName === 'search_products_admin') {
    return await toolFunction(parameters.query, parameters.includeAnalytics);
  } else if (toolName === 'get_business_reports') {
    return await toolFunction(parameters.reportType);
  } else if (toolName === 'generate_product_tags') {
    return await toolFunction(parameters.productName);
  } else if (toolName === 'analyze_feedback') {
    return await toolFunction(parameters.customerMessage);
  } else if (toolName === 'analyze_relationships') {
    return await toolFunction(parameters.field, parameters.value);
  } else if (toolName === 'analyze_invoice') {
    return await toolFunction(parameters.invoiceData);
  } else if (toolName === 'execute_aito_query') {
    return await toolFunction(parameters.queryObject, parameters.endpoint);
  } else if (toolName === 'get_query_examples') {
    return await toolFunction(parameters.category);
  } else if (toolName === 'validate_aito_query') {
    return await toolFunction(parameters.queryObject, parameters.endpoint);
  } else {
    // For tools that need no parameters
